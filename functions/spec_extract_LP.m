function output_tab = spec_extract_LP(PanelVAR_output,targetv_str,targetc_str,spec_vec,target_vec,baseSpec_str)
...
for ii_fnames = 1:n_fnames
    ...
    if isempty(indc)
        ...
        if isempty(indv)
            ...
            targetvar_mat(:,ii_fnames) = PanelVAR_output.(spec_str).LP_mean.avg_tab.(targetall_str)-PanelVAR_output.(baseModel).LP_mean.avg_tab.(targetall_str);
        end
    end
end
