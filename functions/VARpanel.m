function Output=VARpanel(Input)
...
delta_minesota=Input.delta_minesota;
...
delta=Input.delta; % set to one nothing changes, set delta <1 we have discounting
...
LambdaCS_local_store=zeros(effsamp,G^2*P,n_CS);
...
%     while check==0
   ...
   %     very unstable !!!
    ...
    %          end
     ...
     catch %Scalable Approximate MCMC Algorithms
...
%   end
    ...
    %%% minesotaprior
     ...
     if minesotaadaptive==1
            ...
            if G>7
                 LambdaARC1(LambdaARC1>0.1)=0.1;
                 LambdaARC2(LambdaARC2>0.01)=0.01;
    ...
    if CS_local==1||CS_country==1||CS_global==1
        LambdaCS_comb=LambdaCS_local(:).*cell2mat(arrayfun(@(LambdaCS)repmat(LambdaCS,G^2*P,1),LambdaCS,'uni',0))*LambdaCS_global;
        iVbeta=full(iVbeta);
        for kk=1:n_CS
            ...
            iVbeta(  ind_temp1,ind_temp1) =diag(diag(iVbeta(  ind_temp1 , ind_temp1))+ 1./ LambdaCS_comb(  range));
            iVbeta(   ind_temp2, ind_temp2) =diag(diag(iVbeta(   ind_temp2 ,  ind_temp2))+ 1./ LambdaCS_comb(  range));
            iVbeta(ind_temp2,ind_temp1)   =diag(-1./ LambdaCS_comb(  range));
            iVbeta(ind_temp1,ind_temp2)   =diag(-1./ LambdaCS_comb(  range));
            
        end
        iVbeta = sparse(iVbeta);
    end
    ...
    if i > BURNIN && mod(i, thin)== 0 %% save stuff
        ...
        if CS_local==1||CS_country==1||CS_global==1
        LambdaCS_comb_store(:,(i-BURNIN)/thin)=  LambdaCS_comb;
        end
...
Output.LambdaCS_local = squeeze(mean(LambdaCS_local_store));
Output.LambdaCS     = mean(LambdaCS_store,2);
if CS_local==1||CS_country==1||CS_global==1
Output.LambdaCS_comb     = mean(LambdaCS_comb_store,2);
end
...
% plot(aa(:,:,variable)')
% subplot(2,1,2)
% plot(Output.yfor_errorbands(:,:,variable)')
% figure
% plot([aa(5,:,variable)-aa(1,:,variable); Output.yfor_errorbands(5,:,variable)-Output.yfor_errorbands(1,:,variable)]')
% legend
% figure
% plot(Input.Yins(:,variable))

% 
Output.AR_store;
% 
% %plot(LambdaCS_local_store);
% betaAR=inv(Ylagtilde'*Ylagtilde)*Ylagtilde'*y;
% 
%     for nn=1:N 
%         for pp=1:P
%             BetaAR(1+(nn-1)*G:G+(nn-1)*G,1+(nn-1)*G+(pp-1)*G*N:G+(nn-1)*G+(pp-1)*G*N)=  reshape(betaAR(seq+G*(pp-1)+(nn-1)* G*G*P,1),G,G)';
...
%  BetaAR
%               XLiDL = Ylagtilde';%
%         Kbeta = XLiDL*Ylagtilde+iVbeta;%+iVbeta
% 
%         beta_hattilde =Kbeta\( XLiDL*(y));% conjgrad(Kbeta,( XLiDL*(y)));%Kbeta\( XLiDL*(y))
%         BetaAR2=BetaAR;
%             for nn=1:N 
%         for pp=1:P
%             BetaAR2(1+(nn-1)*G:G+(nn-1)*G,1+(nn-1)*G+(pp-1)*G*N:G+(nn-1)*G+(pp-1)*G*N)=  reshape(beta_hattilde(seq+G*(pp-1)+(nn-1)* G*G*P,1),G,G)';
%         end
%     end
%  BetaAR2
%  
...
%        %beta_hattilde = Kbeta\( XLiDL*(y));% numerical unstable sometimes
%        
%         beta_hattilde = Kbeta\( XLiDL*(y));
%         BetaAR3=BetaAR;
%             for nn=1:N 
%         for pp=1:P
%             BetaAR3(1+(nn-1)*G:G+(nn-1)*G,1+(nn-1)*G+(pp-1)*G*N:G+(nn-1)*G+(pp-1)*G*N)=  reshape(beta_hattilde(seq+G*(pp-1)+(nn-1)* G*G*P,1),G,G)';
%         end
%     end
%  BetaAR3
% aa= full(iVbeta)
% iVbeta=iVbeta*100000000
 
%  mean( LambdaCS_local_store)
%   select1=4;
%  select2=3;
%  y=Yins(2:end,select1);
%  x1=Yins(1:end-1,select1);
%  x2=Yins(1:end-1,select2);
% x=[x1 x2];
%  beta=inv(x'*x)*x'*y
%  
%   BetaAR
