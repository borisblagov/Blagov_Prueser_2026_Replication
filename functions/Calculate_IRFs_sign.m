...
rng(1) 
   ...
   % countries       ={'DE','ES','GR'};%,'GR', 'FR','IT','NL','BE','AT','PT','FI'}; %
    common_vars     = {'RGDP' ,'HICP','EURIBOR'};%,%' GOVB10Yd' , 'RGDP' ,'HICP','EURIBOR'
    ...
    minesotaadaptive=1; %1: use standard minesota prior
    minesotafix=1;  
    ...
    Sig_store=zeros(G*N,G*N,effsamp);
    %% Start MCMC %%
   ...
   %median( Lambda_shrinkweigth_store,2)
    ...
    for i=1:nrun
        ...
        if i > BURNIN && mod(i, thin)== 0 %% save stuff
            ...
            LambdaARC1_store((i-BURNIN)/thin,:)=LambdaARC1' ;
            LambdaARC2_store((i-BURNIN)/thin,:)=LambdaARC2';
            Sig_store(:,:,(i-BURNIN)/thin)=Sig;
        end
    end  



restriction=zeros(G,G);
restriction(1,:)=[1 1  1];%demand
restriction(2,:)=[1 -1 0];%supply
restriction(3,:)=[-1 -1 1];%Money
%restriction=restriction';
M=100;

ident=zeros(q,q,effsamp);
for nn=1:N
for ii=1:effsamp
      if mod(ii,1) == 0
         disp([num2str(ii) ' Ident'])
      end
        Sigma=Sig_store(1+G*(nn-1):G+G*(nn-1),1+G*(nn-1):G+G*(nn-1),ii);
      ...
      %restriction=restriction';
       ...
       for i=1:G  
          ...
          for j=i:G
       a= sum(restriction(restriction(:,i)~=0,i).*cand(restriction(:,i)~=0,j)>=0)==size(restriction(restriction(:,i)~=0,i),1);
         if a==0
        a=  sum(-restriction(restriction(:,i)~=0,i).*cand(restriction(:,i)~=0,j)>=0)==size(restriction(restriction(:,i)~=0,i),1);
         cand(:,j)=-cand(:,j);
         end
       ...
       % check if we have same shocks
%        count2=zeros(G,1);
%        for i=1:G
%            count2(i,1)=sum(abs(restriction(:,i)),1);
%        end
%        for j=sum(count2==0)+1:G %assume identified shocks come first
%            for i=1:sum(count2>0)
%                    b= sum(restriction(restriction(:,i)~=0,i).*cand(restriction(:,i)~=0,j)>=0)==size(restriction(restriction(:,i)~=0,i),1);
%          if b==0
%         b=  sum(-restriction(restriction(:,i)~=0,i).*cand(restriction(:,i)~=0,j)>=0)==size(restriction(restriction(:,i)~=0,i),1);
%          cand(:,j)=-cand(:,j);
%          end 
...
%        end
 ...
 if M>1
        ...
        DIFFA0=sum((A0big-repmat(MA0,M,1)).^2,2); %distance from the median
        [~,id]=min(DIFFA0);
 ident(1+G*(nn-1):G+G*(nn-1),1+G*(nn-1):G+G*(nn-1),ii)   =reshape(A0big(id,:),G,G);
else
 ident(1+G*(nn-1):G+G*(nn-1),1+G*(nn-1):G+G*(nn-1),ii)   =reshape(A0big,G,G);
end  
...
%% irfs
 for ii=1:effsamp
               ...
               BetaAR= AR_store(:,:,ii) ;
                ...
                xtp =  zeros(1,q*P);
                for iii=1+P:hor+P %works only for P>0
                    yirf(iii,:)=xtp*BetaAR'+v(iii,:)*ident(:,:,ii);
                    xtp =  [ yirf(iii,:) xtp(1:end-q)];
                end
                
                yirf_store(:,ii,:)=  yirf(P+1:end,:).*standardizesdY;
...
if Save==1
 name=['IRF_Sign_','CS_local=',num2str(CS_local),'CS_country=',num2str( CS_country),'minesota=',num2str(  minesotaadaptive),'Minesotafix=',num2str(minesotafix),'MCMC=',num2str(MCMC),'MCMC=',num2str(BURNIN),'Cplus=',num2str(Minesota_cplus),'.mat']; MCMC
