function [y_out,PHI,N,G,T,Phi_alt,L] = simpvardgp_identical(T)
%--------------------------------------------------------------------------
%   PURPOSE:
...
PHI          = PHI_diff;
A1           = PHI_diff(1:3,1:3);
A2           = PHI_diff(1+G*N:3+G*N,1:3);
A3           = PHI_diff(1+2*G*N:3+2*G*N,1:3);
A4           = PHI_diff(1+3*G*N:3+3*G*N,1:3);
for ii = 1:N
    PHI(1+(ii-1)*G:3+(ii-1)*G,1+(ii-1)*G:3+(ii-1)*G) = A1;
    PHI(1*G*N+1+(ii-1)*G:1*G*N + 3+(ii-1)*G,1+(ii-1)*G:3+(ii-1)*G) = A2;
    PHI(2*G*N+1+(ii-1)*G:2*G*N + 3+(ii-1)*G,1+(ii-1)*G:3+(ii-1)*G) = A3;
    PHI(3*G*N+1+(ii-1)*G:3*G*N + 3+(ii-1)*G,1+(ii-1)*G:3+(ii-1)*G) = A4;
end
