function []=panel4irfdisp(N,n,Units,endo,irf_estimates,strshocks_estimates,IRFperiods,IRFt,stringdates1,T,decimaldates1,pref)
...
for ii=1:N
   ...
   endocell={};
      ...
      for kk=1:n
      % create a header
      header=[{['response of ' Units{ii,1} '_' endo{jj,1} ' to ' Units{ii,1} '_' endo{kk,1} ' shocks']} {''} {''} {''};{''} {''} {''} {''};{''} {'lower bound'} {'median'} {'upper bound'}];
      % complete the cell
      tempcell=[[header;num2cell((1:IRFperiods)') num2cell((irf_estimates{jj,kk,ii})')] vertspace];
...
% write in excel
if pref.results==1
    xlswrite([pref.datapath '\results\' pref.results_sub '.xlsx'],irfcell,'IRF','B2');
end
...
% write in excel
if pref.results==1
    xlswrite([pref.datapath '\results\' pref.results_sub '.xlsx'],strshockcell,'shocks','B2');
end
end
