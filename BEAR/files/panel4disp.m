function []=panel4disp(n,N,m,p,k,T,Yi,Xi,Units,endo,exo,const,beta_gibbs,beta_median,beta_std,beta_lbound,beta_ubound,sigma_gibbs,sigma_median,D_estimates,gamma_estimates,lambda2,lambda3,lambda4,s0,v0,startdate,enddate,forecast_record,forecast_estimates,Fcperiods,stringdates3,Fstartdate,Fcenddate,Feval,Fcomp,data_endo_c,data_endo_c_lags,data_exo_c,It,Bu,IRF,IRFt,pref,names)
...
VARtypeinfo='Panel VAR: random effect model (hierarchical)';
fprintf('%s\n',VARtypeinfo);
fprintf(fid,'%s\n',VARtypeinfo);
...
hyperparam2=['Cross-variable weighting (lambda2):       ' num2str(lambda2)];
fprintf('%s\n',hyperparam2);
fprintf(fid,'%s\n',hyperparam2);

hyperparam3=['lag decay (lambda3):                      ' num2str(lambda3)];
fprintf('%s\n',hyperparam3);
fprintf(fid,'%s\n',hyperparam3);

hyperparam4=['exogenous variable tightness (lambda4):   ' num2str(lambda4)];
fprintf('%s\n',hyperparam4);
fprintf(fid,'%s\n',hyperparam4);

hyperparam5=['IG shape on overall tightness (s0):       ' num2str(s0)];
fprintf('%s\n',hyperparam5);
fprintf(fid,'%s\n',hyperparam5);

hyperparam6=['IG scale on overall tightness (v0):       ' num2str(v0)];
fprintf('%s\n',hyperparam6);
fprintf(fid,'%s\n',hyperparam6);
...
fprintf('%s\n',sigmainfo);
fprintf(fid,'%s\n',sigmainfo);
sigmamat=reshape(sigma_median(:,:,hh),n,n);
% calculate the (integer) length of the largest number in sigma, for formatting purpose
width=length(sprintf('%d',floor(max(abs(vec(sigmamat))))));
...
temp=[];
   ...
   number=num2str(sigmamat(ii,jj),'% .3f');
