...
sourcefile=[pwd '\results panel.xlsx'];
...
% write on excel file
if pref.results==1
    xlswrite([pref.datapath '\results\' pref.results_sub '.xlsx'],generalinfo,'estimation info','C2');
end
...
if CF==1
   ...
   % write on excel file
   if pref.results==1
    xlswrite([pref.datapath '\results\' pref.results_sub '.xlsx'],cfcell,'cond forecasts prior','B2');
   end
end
...
if (F==1 || CF==1)
   ...
   % write on excel file
   if pref.results==1
    xlswrite([pref.datapath '\results\' pref.results_sub '.xlsx'],pexocell,'predicted exo','B2'); 
   end
   end
end
