...
if (VARtype==2 || VARtype==3 || VARtype==4 || VARtype==5 || VARtype==6|| VARtype==7) % && IRF==1 && IRFt==4
...
if (VARtype==2 || VARtype==3 || VARtype==5 || VARtype==6||VARtype==7) && CF==1
...
elseif window_size>0
    numt = length(stringdates1)-window_size+lags-1; % number of different dateroll dates
end
...
for iteration=1:numt % beginning of forecasting loop
    ...
    if window_size>0
        ...
        % original code that uses a rolling window
...
%         %end
        % New code that uses an expanding window
         data_endo = data_endo_full(1:window_size+iteration,:);
        if VARtype==3
            Dmatrix = Dmatrix_full(1:window_size+iteration,:);
            data_endo_a = data_endo_a_full(1:window_size+iteration,:);
        end       
...
%         Fstartlocation1=find(strcmp(names(1:end,1),startdateini))+iteration-1;
        % New code that uses an expanding window
        Fstartlocation1=find(strcmp(names(1:end,1),startdateini));
    ...
    if VARtype==4
        ...
        if F==1
            ...
            if Feval == 1
                if panel == 3
               Feval_unit=  panel_feval_boris(n,N,m,p,k,T,Ymat,Xmat,Units,endo,exo,const,beta_gibbs,B_median,beta_median,beta_std,beta_lbound,beta_ubound,sigma_gibbs,...
                ...
                elseif panel == 4
                   Feval_unit=  panel_feval_boris(n,N,m,p,k,T,Yi,Xi,Units,endo,exo,const,beta_gibbs,sigma_gibbs,...
                    forecast_record,forecast_estimates,Fcperiods,...
                    stringdates3,Fstartdate,Fcenddate,Feval,Fcomp,data_endo_c,data_endo_c_lags,data_exo_c,It,Bu,IRF,IRFt,pref,names);
    ...
    if VARtype == 7
        ...
        %% BLOCK 8: historical decomposition            THIS PART WORKS. It is coded to be done at the median of the monthly gdp estimates
