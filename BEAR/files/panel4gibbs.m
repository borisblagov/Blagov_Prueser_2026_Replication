function [beta_gibbs sigma_gibbs]=panel4gibbs(N,n,h,T,k,q,Yi,Xi,s0,omegab,v0,It,Bu,pick,pickf)
...
b=betam+chol(nspd((1/N)*sigmab),'lower')*mvnrnd(zeros(q,1),eye(q),1)';
...
% draw beta
beta(:,ii)=betabar+chol(nspd(omegabar),'lower')*mvnrnd(zeros(q,1),eye(q),1)';
end
