...
% VAR model selected (1=OLS VAR, 2=BVAR, 3=mean-adjusted BVAR, 4=panel Bayesian VAR, 5=Stochastic volatility BVAR, 6=Time varying, 7=MF-VAR)
VARtype=7;
...
if VARtype == 2  
    ...
    varendo='ipi ifosurvmanuf6m orders ygdp';
...
%     pref.datapath='C:\Users\boris\Dropbox\BEAR';
    pref.datapath='F:\Dropbox\BEAR';
    % excel results file name
    pref.results_sub='results_Bvar2';
...
if VARtype == 7
  % data frequency (1=yearly, 2= quarterly, 3=monthly, 4=weekly, 5=daily, 6=undated)
    ...
    enddate='2020m12';
    % endogenous variables; must be a single string, with variable names separated by a space
    varendo='unemp	ipi	ppi	empl	ifosurvmanuf6m	cpi	orders	eurib	govb10y	dax30	gdp	gva_ind	gva_cons	gva_hot	gva_it	gva_fin	gva_estate	gva_bserv	gva_health	gva_serv';
    % exogenous variables, if any; must be a single string, with variable names separated by a space
    varexo='';
    % number of lags
    lags=6;
...
%     pref.datapath='C:\Users\boris\Dropbox\BEAR';
    pref.datapath='F:\Dropbox\BEAR';
    % excel results file name
    pref.results_sub='results_mf_var';
    % to output results in excel
    pref.results=0;
    % output charts
    pref.plot=1;
