function []=bvardisp(beta_median,beta_std,beta_lbound,beta_ubound,sigma_median,log10ml,dic,X,Y,n,m,p,k,q,T,prior,bex,hogs,lrp,H,ar,lambda1,lambda2,lambda3,lambda4,lambda5,lambda6,lambda7,lambda8,IRFt,const,beta_gibbs,endo,exo,startdate,enddate,stringdates1,decimaldates1,pref,scoeff,iobs)
...
fprintf(fid,'%s\n','%    The authors are grateful to Marta Banbura, Fabio Canova,           %');
fprintf('%s\n','%    Matteo Ciccarelli, Marek Jarocinski, Carlos Montes-Galdon,         %');
fprintf(fid,'%s\n','%    Matteo Ciccarelli, Marek Jarocinski, Carlos Montes-Galdon,         %');
fprintf('%s\n','%    Michele Lenza, Chiara Osbat, Mirela Miescu, Gary Koop, Giorgio Primiceri %');
fprintf(fid,'%s\n','%    Michele Lenza, Chiara Osbat, Mirela Miescu, Gary Koop, Giorgio Primiceri, %');
fprintf('%s\n','%    Michal Rubaszek and Barbara Rossi and Francesca Loria.  %');
fprintf(fid,'%s\n','%    Michal Rubaszek and Barbara Rossi and Francesca Loria %');
...
fprintf('%s\n','%                                                                       %');
fprintf(fid,'%s\n','%                                                                       %');
fprintf('%s\n','%   These programmes are the responsibilities of the authors and not of the ECB and  all   %'); 
fprintf(fid,'%s\n','%   These programmes are the responsibilities of the authors and not of the ECB and all   %'); 
...
if prior==11||prior==12||prior==13||prior==31||prior==32||prior==41
...
set(postdis,'name','posterior ditribution of VAR coefficients');
   ...
   set(gca,'FontName','Times New Roman');
      ...
      % side labels
      if rem((ii-1)/k,1)==0
      ylabel(endo{(ii-1)/k+1,1},'FontWeight','normal');
      end
   end
end
