function [ar lambda1 lambda3 lambda4 lambda6 lambda7]=nwgridsearch(X,Y,n,m,p,k,q,T,lambda2,lambda5,lambda6,lambda7,grid,arvar,data_endo,data_exo,prior,hogs,bex,const,scoeff,iobs,pref,It,Bu)
   ...
   for jj=grid{2,1}:grid{2,3}:grid{2,2}
      ...
      for kk=grid{4,1}:grid{4,3}:grid{4,2}
         ...
         % now the treatment will differ depending on whether there are dummy observations or not in the model
            ...
            [Bbar betabar phibar Sbar alphabar alphatilde]=nwpost(B0,phi0,S0,alpha0,X,Y,n,T,k);
            % obtain the log marginal value (up to a constant) term
            [logml]=nwmlikgrid(X,n,k,phi0,S0,Sbar,alphabar);
            ...
            elseif scoeff==1 && iobs==0
               ...
               [Ystar ystar Xstar Tstar Ydum ydum Xdum Tdum]=gendummy(data_endo,data_exo,Y,X,n,m,p,T,const,mm,lambda7,scoeff,iobs);
               % obtain prior elements
               [B0 beta0 phi0 S0 alpha0]=nwprior(ii,arvar,jj,kk,ll,n,m,p,k,q,prior);
               ...
               [logml]=nwmlikgrid(Xstar,n,k,phi0,S0,Sbar,alphabar);
               ...
               [logmldum]=nwmlikgrid(Xdum,n,k,phi0,S0,Sbardum,alphabardum);
            ...
            elseif scoeff==0 && iobs==1
               ...
               % generate the dummy observations
               [Ystar ystar Xstar Tstar Ydum ydum Xdum Tdum]=gendummy(data_endo,data_exo,Y,X,n,m,p,T,const,lambda6,mm,scoeff,iobs);
               % obtain prior elements
               [B0 beta0 phi0 S0 alpha0]=nwprior(ii,arvar,jj,kk,ll,n,m,p,k,q,prior);
               % obtain posterior elements for the dummy-augmented data
               [Bbar betabar phibar Sbar alphabar alphatilde]=nwpost(B0,phi0,S0,alpha0,Xstar,Ystar,n,Tstar,k);
            ...
            elseif scoeff==1 && iobs==1
               ...
               for mm=grid{6,1}:grid{6,3}:grid{6,2}
                  ...
                  % generate the dummy observations
                  [Ystar ystar Xstar Tstar Ydum ydum Xdum Tdum]=gendummy(data_endo,data_exo,Y,X,n,m,p,T,const,mm,nn,scoeff,iobs);
                  % obtain prior elements
                  [B0 beta0 phi0 S0 alpha0]=nwprior(ii,arvar,jj,kk,ll,n,m,p,k,q,prior);
                  % obtain posterior elements for the dummy-augmented data
                  [Bbar betabar phibar Sbar alphabar alphatilde]=nwpost(B0,phi0,S0,alpha0,Xstar,Ystar,n,Tstar,k);
