function [ar lambda1 lambda2 lambda3 lambda4 lambda6 lambda7]=mgridsearch(X,Y,y,n,m,p,k,q,T,lambda5,lambda6,lambda7,grid,arvar,sigmahat,data_endo,data_exo,prior,hogs,bex,blockexo,const,scoeff,iobs,pref,It,Bu)
         ...
         for ll=grid{4,1}:grid{4,3}:grid{4,2}
            ...
            % now the treatment will differ depending on whether there are dummy observations or not in the model
               ...
               elseif scoeff==1 && iobs==0
                  ...
                  [Ystar ystar Xstar Tstar Ydum ydum Xdum Tdum]=gendummy(data_endo,data_exo,Y,X,n,m,p,T,const,nn,lambda7,scoeff,iobs);
                  % obtain prior elements
                  [beta0 omega0 sigma]=mprior(ii,arvar,sigmahat,jj,kk,ll,mm,lambda5,n,m,p,k,q,prior,bex,blockexo);
               ...
               elseif scoeff==0 && iobs==1
                  ...
                  % generate the dummy observations
                  [Ystar ystar Xstar Tstar Ydum ydum Xdum Tdum]=gendummy(data_endo,data_exo,Y,X,n,m,p,T,const,lambda6,nn,scoeff,iobs);
                  % obtain prior elements
                  [beta0 omega0 sigma]=mprior(ii,arvar,sigmahat,jj,kk,ll,mm,lambda5,n,m,p,k,q,prior,bex,blockexo);
                  % obtain posterior elements for the dummy-augmented data
                  [betabar omegabar]=mpost(beta0,omega0,sigma,Xstar,ystar,q,n);
               ...
               elseif scoeff==1 && iobs==1
                  ...
                  for nn=grid{6,1}:grid{6,3}:grid{6,2}
                     ...
                     % generate the dummy observations
                     [Ystar ystar Xstar Tstar Ydum ydum Xdum Tdum]=gendummy(data_endo,data_exo,Y,X,n,m,p,T,const,nn,oo,scoeff,iobs);
                     % obtain prior elements
                     [beta0 omega0 sigma]=mprior(ii,arvar,sigmahat,jj,kk,ll,mm,lambda5,n,m,p,k,q,prior,bex,blockexo);
                     % obtain posterior elements for the dummy-augmented data
                     [betabar omegabar]=mpost(beta0,omega0,sigma,Xstar,ystar,q,n);
