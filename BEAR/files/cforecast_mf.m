function [cforecast_record]=cforecast_mf(data_endo_a,data_exo_a,data_exo_p,It,Bu,Fperiods,cfconds,cfshocks,cfblocks,CFt,const,beta_gibbs,D_record,gamma_record,n,m,p,k,q)
...
% inputs:  - matrix 'data_endo_a': a 3D matrix storing the pre-forecast endogenous data. For an explanation of the third dimension see below the section "Changes with respect ot the version
...
% Changes with respect to the version without the "mf" subscript by Boris Blagov
% - on line 54 we change ...(data_endo_a,...) with ...(data_endo_a(:,:,ii),...). This reflects
% the fact that data_endo_a is a matrix of data with a third dimensions: draws of the data. For some months we don't know the true value, so we have It-Bu 
% draws of the monthly value and we have to reflect that uncertainty. One could alternatively just use "median(data_endo_a,3)" and disregard that uncertainty
...
fmat=forecastsim(data_endo_a(:,:,ii),data_exo_p,beta,n,p,k,Fperiods);
